const { Pool } = require('pg');
const jwt      = require('jsonwebtoken');

const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });

module.exports = async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  if (req.method !== 'GET') return res.status(405).end();

  // Support both: JWT auth (polling after payment) and ?reference= (admin/webhook use)
  const { reference } = req.query;
  if (reference) {
    const { rows } = await pool.query(
      'SELECT payment_status, payment_verified_at FROM users WHERE payment_reference = $1', [reference]);
    if (!rows.length) return res.status(404).json({ error: 'Reference not found' });
    return res.json(rows[0]);
  }

  // JWT-based lookup (used by pending-payment.html polling)
  const auth = req.headers.authorization || '';
  if (!auth.startsWith('Bearer ')) return res.status(401).json({ error: 'Unauthenticated' });
  try {
    const payload = jwt.verify(auth.slice(7), process.env.JWT_SECRET);
    const { rows } = await pool.query(
      'SELECT payment_status, payment_verified_at FROM users WHERE id = $1', [payload.id]);
    if (!rows.length) return res.status(404).json({ error: 'User not found' });
    return res.json(rows[0]);
  } catch {
    return res.status(401).json({ error: 'Invalid or expired token' });
  }
};
